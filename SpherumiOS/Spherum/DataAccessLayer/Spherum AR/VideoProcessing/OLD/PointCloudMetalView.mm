...
//    NSLog(@"CVPixelBufferGetPixelFormatType: %u", (unsigned int)CVPixelBufferGetPixelFormatType(depthFrame));
//    NSLog(@"Is DepthFloat16: %d", CVPixelBufferGetPixelFormatType(depthFrame) == kCVPixelFormatType_DepthFloat16);
...
-(simd::float4x4)getFinalViewMatrix {
    ...
    simd::float4x4 rotation = simd_float4x4();
    
    rotation.columns[0] = { 0, -1, 0, 0 };
    rotation.columns[1] = {-1, 0, 0, 0 };
    rotation.columns[2] = { 0, 0, 1, 0 };
    rotation.columns[3] = { 0, 0, 0, 1 };
    
    simd::float4x4 scale = simd_float4x4();
    scale.columns[0] = { aspect, 0, 0, 0 };
    scale.columns[1] = {  0, 1/aspect, 0, 0 };
    scale.columns[2] = {  0, 0, 1, 0 };
    scale.columns[3] = {  0, 0, 0, 1 };
    
    // Final view matrix is projection * view * model. In our case, we never move the model itself, so we can ignore its matrix.
    return rotation * scale * appleProjMat * appleViewMat;
}
